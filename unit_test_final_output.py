...
class TestOutput(unittest.TestCase):
    ...
    def test_df_vlans_loader(self):
        result=main_script_functions.load_file('test_vlans.csv')
    ...
    def test_df_requests_loader(self):
        result=main_script_functions.load_file('test_requests.csv')
        test_result=len(result)
        self.assertNotEqual(test_result, 0)
    
    # This test will see if the primary port filter is working
    def test_primary_port_filter(self):
        result=main_script_functions.vlans_df_primary_port_sorter('test_vlans.csv',1)
    ...
    def test_non_primary_port_filter(self):
        result=main_script_functions.vlans_df_primary_port_sorter('test_vlans.csv',0)
    ...
    def test_redundant_df_filter(self):
        result=main_script_functions.redundant_df_loader_sorter('test_requests.csv',1)
    ...
    def test_non_redundant_df_filter(self):
        result=main_script_functions.redundant_df_loader_sorter('test_requests.csv',0)
    ...
    def test_vlans_length(self):
        result_1=main_script_functions.vlans_df_primary_port_sorter('test_vlans.csv',0)
        result_2=main_script_functions.vlans_df_primary_port_sorter('test_vlans.csv',1)
        combined_length=len(result_1)+len(result_2)
        length_input=len(main_script_functions.load_file('test_vlans.csv'))
    ...
    def test_requests_length(self):
        result_1=main_script_functions.redundant_df_loader_sorter('test_requests.csv',0)
        result_2=main_script_functions.redundant_df_loader_sorter('test_requests.csv',1)
        combined_length=len(result_1)+len(result_2)
        length_input=len(main_script_functions.load_file('test_requests.csv'))
        self.assertEqual(combined_length,length_input)
    ...
    def test_redundant_df_parser_length(self):
        #length_non_redundant=len(main_script_functions.redundant_df_loader_sorter('test_requests.csv',1))
        redundant_df=main_script_functions.redundant_df_loader_sorter('test_requests.csv',1)
        non_redundant_df=main_script_functions.redundant_df_loader_sorter('test_requests.csv',0)
        vlans_df_primary_port=main_script_functions.vlans_df_primary_port_sorter('test_vlans.csv',1)
        length_original_primary_port_df=len(vlans_df_primary_port)
        vlans_df_primary_port_used=main_script_functions.vlans_df_primary_port_sorter('test_vlans.csv',1)
        vlans_df_non_primary_port=main_script_functions.vlans_df_primary_port_sorter('test_vlans.csv',0)
        #for i in range(0,10):
    ...
    def test_final_df_length(self):
        length_1=len(main_script_functions.load_file('test_requests.csv'))
        length_non_redundant=len(main_script_functions.redundant_df_loader_sorter('test_requests.csv',1))
        redundant_df=main_script_functions.redundant_df_loader_sorter('test_requests.csv',1)
        non_redundant_df=main_script_functions.redundant_df_loader_sorter('test_requests.csv',0)
        vlans_df_primary_port=main_script_functions.vlans_df_primary_port_sorter('test_vlans.csv',1)
    ...
    def test_final_df_values(self):
        ...
        final_non_redundant_df=final_non_redundant_df_list[3]
        final_output_df=final_redundant_df.append(final_non_redundant_df, ignore_index=True).sort_values(['request_id', 'primary_port', 'device_id'], ascending=[True, True, True]).reset_index(drop=True).drop(['redundant'], axis=1)[['request_id','device_id','primary_port','vlan_id']]
        test_output_df=main_script_functions.load_file('test_output.csv')
